...
while run:
    ...
    bg = PhotoImage(file="Forest_BG2.png")
    ...
    for i in range(0,len(selected_word)):
        exec('d{}=Label(window,text="_",bg="#0E8200",font=("arial",40))'.format(i))
    ...
    hmi = ['1','2','3','4','5','6','7']
    for hangman in hmi:
        exec('h{}=PhotoImage(file="{}.png")'.format(hangman,hangman))
   ...
   #letters placement
    ...
    for q1 in button:
        exec('{}=Button(window,bd=0,command=lambda:check("{}","{}"),bg="#0E8200",activebackground="#E7FFFF",font=10,image={})'.format(q1[0],q1[1],q1[0],q1[1]))
        exec('{}.place(x={},y={})'.format(q1[0],q1[2],q1[3]))
        
    #hangman placement
    han = [['c1','h1'],['c2','h2'],['c3','h3'],['c4','h4'],['c5','h5'],['c6','h6'],['c7','h7']]
    for p1 in han:
        exec('{}=Label(window,bg="#000000",image={})'.format(p1[0],p1[1]))
    ...
    ex = Button(window,bd = 0,command = close,bg="#209304",activebackground = "#96AE31",font = 10,image = e1)
    ex.place(x=770,y=10)

    #score display
    s2 = 'SCORE:'+str(score)
    s1 = Label(window,text = s2,bg = "#209304",font = ("jokerman",25))
    s1.place(x = 10,y = 10)
